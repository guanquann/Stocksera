...
def get_next_retail_sales_date():
    ...
    """
    df = tabula.read_pdf(
        r"https://www.census.gov/retail/marts/www/martsdates.pdf", pages=1
    )[0]
...
def get_next_cpi_date():
    ...
    df = pd.read_html(
        requests.get(
            r"https://www.bls.gov/schedule/news_release/cpi.htm", headers=header
        ).text
    )[0][:-1]
    df["Release Date"] = pd.to_datetime(df["Release Date"], errors="coerce")
    df = df[df["Release Date"] >= current_date].iloc[0]
...
def get_holidays():
    ...
    """
    holidays_df = pd.read_html(r"https://www.sec.gov/edgar/filer-information/calendar")[
        0
    ]
...
def main():
    ...
    cur.execute(
        "SELECT record_date from reverse_repo ORDER BY record_date DESC LIMIT 1"
    )
    record_date = cur.fetchone()
    rrp_treasury_date = get_next_rrp_treasury_date(
        datetime.strptime(record_date[0], "%Y-%m-%d") + timedelta(days=1)
    )
    ...
    with open(r"database/economic_date.json", "w") as r:
        information = {
            "Retail Sales": {
                "Ref Month": retail_df["Data Month"],
                "Release Date": retail_df["Release Date"],
            },
            "Inflation": {
                "Ref Month": cpi_df["Reference Month"],
                "Release Date": cpi_df["Release Date"],
            },
