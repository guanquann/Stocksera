...
def main():
    """
    Get Fear and Greed from CNN
    """
    print("Getting CNN fear and greed...")

    data = requests.get(
        "https://production.dataviz.cnn.io/index/fearandgreed/graphdata", headers=header
    ).json()

    df = pd.DataFrame(data["fear_and_greed_historical"]["data"])

    df["x"] = df["x"].apply(
        lambda x: str(datetime.fromtimestamp(int(str(x)[:-5]))).split()[0]
    )
    ...
    cur.executemany(
        "INSERT IGNORE INTO fear_and_greed VALUES (%s, %s, %s, %s)", df.values.tolist()
    )
