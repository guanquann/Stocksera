...
def get_30d_data_finra():
    ...
    while current_date >= last_date:
        url = r"https://cdn.finra.org/equity/regsho/daily/CNMSshvol{}.txt".format(
            str(last_date).replace("-", "")
        )
    ...
    while start < len(combined_df):
        cur.executemany(
            "INSERT IGNORE INTO short_volume VALUES (%s, %s, %s, %s, %s, %s)",
...
def get_daily_data_finra(
    date_to_process: datetime.date = datetime.utcnow().date() - timedelta(days=1),
):
    """
    Get short volume data from https://cdn.finra.org/
    """
    url = r"https://cdn.finra.org/equity/regsho/daily/CNMSshvol{}.txt".format(
        str(date_to_process).replace("-", "")
    )
    s = requests.get(url).content
    df = pd.read_csv(io.StringIO(s.decode("utf-8")), delimiter="|")
    if len(df) == 1:
        ...
        df["Date"] = (
            df["Date"].astype(str).apply(lambda x: x[0:4] + "-" + x[4:6] + "-" + x[6:])
        )
        df["%Shorted"] = 100 * (df["ShortVolume"] / df["TotalVolume"])
        df["%Shorted"] = df["%Shorted"].round(2)
        ...
        stats_df = stats_df[
            ["symbol", "marketCap", "changesPercentage", "previousClose", "volume"]
        ]
        stats_df = stats_df.rename(
            columns={
                "marketCap": "Market Cap",
                "changesPercentage": "1 Day Change %",
        ...
        highest_shorted.to_sql(
            "highest_short_volume", engine, if_exists="replace", index=False
        )
