...
def main(start_date=datetime.utcnow() - timedelta(days=5), end_date=datetime.utcnow()):
    ...
    while start_date != end_date:
        ...
        nyse_df = pd.read_csv(
            f"https://www.nyse.com/api/regulatory/threshold-securities/download?"
            f"selectedDate={formatted_date}",
            delimiter="|",
        )[:-1]
        ...
        try:
            nasdaq_df = pd.read_fwf(
                f"http://www.nasdaqtrader.com/dynamic/symdir/regsho/nasdaqth{formatted_date}.txt",
                error_bad_lines=False,
            )[:-1]
            nasdaq_df["Symbol"] = (
                nasdaq_df["Symbol|Security Name|Market Category|Reg SHO"]
                .astype(str)
        ...
        cur.executemany(
            "INSERT IGNORE INTO threshold_securities VALUES (%s, %s)",
            df.values.tolist(),
        )
        cnx.commit()
