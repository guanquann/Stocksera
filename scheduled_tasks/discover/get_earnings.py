...
def main():
    ...
    while current_index != 21:
        df = pd.DataFrame(
            finnhub_client.earnings_calendar(
                _from=str(current_date + timedelta(days=current_index)),
    ...
    mkt_cap_df = get_ticker_list_stats(main_df["symbol"].tolist())[
        ["symbol", "marketCap"]
    ]
    ...
    cur.executemany(
        "INSERT IGNORE INTO earnings VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
